...
def main():
    ...
    # Join with targets
    rubies = join(targets, results, keys=['root', 'uid'])
    ...
    grade3 = df.loc[
        df['grade'] == 3, ['root', 'uid', 'WAIC_broad', 'WAIC_narrow', 'WAIC']
    ].drop_duplicates()
    ...
    grade3 = grade3.sort_values(
        ['pbroad', 'root', 'uid'], ascending=[False, True, True]
    )
    ...
    # Merge
    df_grade3 = df.merge(
        grade3, on=['root', 'uid', 'WAIC_broad', 'WAIC_narrow', 'WAIC']
    )

    # Sort by root, uid, pbroad
    df_grade3 = df_grade3.sort_values(
        ['pbroad', 'root', 'uid'], ascending=[False, True, True]
    )

    # Save output
    out = df_grade3.drop_duplicates(subset=['root', 'uid'])
    out = Table.from_pandas(
        out[['index', 'root', 'uid', 'z', 'zfit', 'pbroad', 'pcauchy']]
    )
    ...
    with Pool(args.ncpu) as pool:
        pool.map(plot, df_grade3.groupby(['root', 'uid']))

    # for row in df_grade3.groupby(['root', 'uid']):
    #     plot(row)
...
def plot(row):
    ...
    # if os.path.exists(f'Comparison-PDF/{root}-{uid}_comparison.pdf'):
    #     return
    ...
    with np.load(f'RUBIES/Results/{root}-{uid}_full.npz', 'rb') as f:
        narrow = {key: f[key] for key in f}
    with np.load(f'RUBIES/Results/{root}-{uid}_broad_full.npz', 'rb') as f:
    ...
    # Set labels
    fig.suptitle(
        rf'{root}-{uid}: ($z = {redshift_initial:.3f}$), FWHW-single: {fwhm:.0f} $\pm$ {fwhm_err:.0f}, FWHM-narrow: {fwhm_narrow:.0f} $\pm$ {fwhm_narrow_err:.0f}, FWHM-broad: {fwhm_broad:.0f} $\pm$ {fwhm_broad_err:.0f}, P(BL) = {pbroad:.2f}'
    )
    fig.supylabel(rf'$f_\lambda$ [{flux.unit:latex_inline}]')
    fig.supxlabel(rf'$\lambda$ [{u.um:latex_inline}]')

    # Save figure
    fig.savefig(f'Comparison-PDF/{data["index"].iloc[0]}-{root}-{uid}_comparison.pdf')
    fig.savefig(
        f'Comparison-JPG/{data["index"].iloc[0]}-{root}-{uid}_comparison.jpg', dpi=150
    )
    pyplot.close(fig)
